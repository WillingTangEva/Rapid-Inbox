...
脚本会自动创建 `.venv`、安装依赖、复制 `.env.example`、构建 C++ `rapid-inbox-ingestd`，并启动 Python HTTP + C++ SMTP 收件入口。默认绑定：
...
```bash
sudo apt-get update
sudo apt-get install -y python3 python3-venv python3-pip cmake g++ libsqlite3-dev libssl-dev
```
