...
usage() {
    cat <<'EOF'
Usage: bash quickstart.sh [--python-smtp] [--http-port PORT] [--smtp-port PORT] [--no-install]
...
if [ "$USE_CPP_INGESTD" -eq 1 ]; then
    ...
    HTTP_PID=$!
    "$ROOT_DIR/cpp/ingestd/build/rapid-inbox-ingestd" --base-dir "$ROOT_DIR" > "$RUN_DIR/ingestd.log" 2>&1 &
    INGESTD_PID=$!
else
